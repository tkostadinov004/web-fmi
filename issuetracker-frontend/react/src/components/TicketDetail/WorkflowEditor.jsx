...
const WorkflowEditor = ({ projectId, canEdit, onClose }) => {
  ...
  const handleCheckboxChange = (sourceStatus, targetStatus, isChecked) => {
    setWorkflow((prev) => {
      ...
      if (isChecked) {
        ...
        updatedTransitions = updatedTransitions.filter((t) => !(t.source === sourceStatus && t.target === targetStatus));
      }
  ...
  const activeTransitions = new Set(workflow?.transitions.map((t) => `${t.source}-${t.target}`) || []);
  ...
  return (
    <div className="workflow-editor-container">
      ...
      <div className="workflow-initial-status" style={{ marginBottom: "15px" }}>
        <label>
          <strong>Начален статус на нов билет: </strong>
        </label>
        <select value={workflow.initialStatus} onChange={(e) => setWorkflow({ ...workflow, initialStatus: e.target.value })} disabled={!isEditMode || isSaving}>
      ...
      <WorkflowMatrix workflow={workflow} activeTransitions={activeTransitions} isEditMode={isEditMode} isSaving={isSaving} onCheckboxChange={handleCheckboxChange} />

      {isEditMode && (
        <div className="workflow-add-status" style={{ marginTop: "15px", display: "flex", gap: "10px" }}>
          <input type="text" placeholder="Име на нов статус (напр. QA)" value={newStatusText} onChange={(e) => setNewStatusText(e.target.value)} disabled={isSaving} />
      ...
      <div className="workflow-actions" style={{ marginTop: "20px" }}>
        {!isEditMode && (
          <>
            <button className="edit-workflow-btn" onClick={() => setIsEditMode(true)} disabled={!canEdit} title={!canEdit ? "Нямате права да редактирате workflow-а" : "Редактирай"}>
              Edit Workflow
            </button>
