...
const EditableTitle = ({ projectId, ticketCode, initialTitle, onTitleUpdate }) => {
  ...
  if (isEditing) {
    return (
      <div className="editable-title-container">
        <input type="text" className="title-input" value={titleText} onChange={(e) => setTitleText(e.target.value)} onKeyDown={handleKeyDown} autoFocus disabled={isSaving} />
  ...
  return (
    <h1 className="title-display" onClick={() => setIsEditing(true)} title="Кликни за редакция">
      {initialTitle}
    </h1>
  );
};
